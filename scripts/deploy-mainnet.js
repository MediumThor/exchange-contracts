...
async function main() {
    ...
    const factory = await deploy("ArcanumFactory", [deployer.address]);
    const router = await deploy("ArcanumRouter", [
        factory.address,
        nativeToken,
    ]);
    const chef = await deploy("MiniChefV2", [arc.address, deployer.address]);
    const treasury = await deploy("CommunityTreasury", [arc.address]);
    const staking = await deploy("StakingRewards", [arc.address, arc.address]);
    ...
    // Deploy Revenue Distributor (Joint treasury of ARC and FARC)
    ...
    const feeCollector = await deploy("ArcanumFeeCollector", [
        staking.address,
        router.address,
        chef.address,
    ...
    await arc.setMinter(vester.address);
    await confirmTransactionCount();
    console.log("Transferred ARC minter role to TreasuryVester.");

    await arc.setAdmin(timelock.address);
    await confirmTransactionCount();
    console.log("Transferred ARC ownership to Timelock.");
    ...
    // deposit dummy ARL for the fee collector
    ...
    await factory.createPair(arc.address, nativeToken);
    await confirmTransactionCount();
    var arcPair = await factory.getPair(arc.address, nativeToken);
    await chef.addPool(
        WETH_ARC_FARM_ALLOCATION,
        arcPair,
        ethers.constants.AddressZero
    ...
    await confirmTransactionCount();
    console.log("Added MiniChef pool 1 for WAVAX-ARC.");
