...
describe('Governance', function () {
    before(async function () {
        ...
        this.AltCoin = await ethers.getContractFactory("Arc");
    ...
    beforeEach(async function () {
        // ARC
        this.arc = await this.ARC.deploy(OWNER_ADDRESS);
        await this.arc.deployed();
        this.arcHandle2 = await this.arc.connect(this.addr2);
        this.arcHandle3 = await this.arc.connect(this.addr3);
        ...
        // GovernorAlpha
        this.governor = await this.Governor.deploy(this.timelock.address, this.arc.address, OWNER_ADDRESS);
        ...
        // TreasuryVester
        this.treasury = await this.TreasuryVester.deploy(this.arc.address);
        await this.treasury.deployed();

        // Community Treasury
        this.community = await this.Community.deploy(this.arc.address);
        ...
        this.lpManager = await this.LpManager.deploy(this.wavax.address, this.arc.address,
    ...
    describe("Timelock Proposals", function () {
        ...
        it('Spend From Community Treasury', async function () {
            ...
            // Check preconditions
            await this.arc.transfer(this.community.address, transferAmount);
            expect(await this.community.balance()).to.equal(transferAmount);
            expect(await this.arc.balanceOf(UNPRIVILEGED_ADDRESS)).to.equal(0);
            ...
            await ethers.provider.send("evm_setNextBlockTimestamp", [updateTime]);
            await this.timelock.executeTransaction(target, value, sig, callData, startTime + DELAY);
            expect(await this.arc.balanceOf(UNPRIVILEGED_ADDRESS)).to.equal(transferAmount);
    ...
    describe("Governor Proposals", function () {
        it('Change Admin', async function () {
            ...
            // Transfer ARC
            await this.arc.transfer(this.addr2.address, this.proposalThreshold);
            await this.arc.transfer(this.addr3.address, this.voteThreshold);

            // Delegate ARC so addresses can vote, must delegate to themselves
            await this.arcHandle2.delegate(this.addr2.address);
            await this.arcHandle3.delegate(this.addr3.address);
            await this.arc.delegate(OWNER_ADDRESS);
