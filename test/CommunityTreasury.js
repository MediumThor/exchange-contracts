...
describe('CommunityTreasury', function () {
    ...
    describe("Constructor", function () {
        it('ARC', async function () {
            expect(await this.community.arc()).to.equal(this.arc.address);
        });
    ...
    describe("Transfer", function () {
        it('Owner can transfer', async function () {
            ...
            expect(await this.arc.balanceOf(this.community.address)).to.equal(transferAmount);
            expect(await this.arc.balanceOf(this.addr2.address)).to.equal(0);

            await this.community.transfer(this.addr2.address, transferAmount);
            expect(await this.arc.balanceOf(this.addr2.address)).to.equal(transferAmount);
        });
        ...
        it('No balance to transfer', async function () {
            const transferAmount = oneToken.mul(100);
            expect(await this.arc.balanceOf(this.community.address)).to.equal(0);
