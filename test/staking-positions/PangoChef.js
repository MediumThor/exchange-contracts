...
const ARC_SUPPLY = ethers.utils.parseUnits("500000000", 18);
const ZERO_ADDRESS = ethers.constants.AddressZero;
const PERIOD_DURATION = 86400;

// Start test block
describe("ArcanaChef.sol", function () {
  before(async function () {
    ...
    this.Arcanum = await ethers.getContractFactory("Arc");
    this.Factory = await ethers.getContractFactory("ArcanumFactory");
    this.Chef = await ethers.getContractFactory("ArcanaChef");
    this.Wavax = await ethers.getContractFactory("WAVAX");
    this.Pair = await ethers.getContractFactory("ArcanumPair");
    this.Router = await ethers.getContractFactory("ArcanumRouter");
  });
  ...
  beforeEach(async function () {
    ...
    // Deploy ARC ERC20 token for rewards.
    this.arc = await this.Arcanum.deploy(ARC_SUPPLY, ARC_SUPPLY, "ARC", "Arcanum");
    await this.arc.deployed();
    this.alt_arc = await this.arc.connect(this.unauthorized);

    // Deploy another ERC20 token.
    this.another_token = await this.Arcanum.deploy(ARC_SUPPLY, ARC_SUPPLY, "ARC", "Arcanum");
    ...
    // Create ARC-WAVAX pair.
    await this.factory.createPair(this.wavax.address, this.arc.address);
    let arl_address = await this.factory.getPair(this.wavax.address, this.arc.address);
    this.arl = await this.Pair.attach(arl_address);
    this.alt_arl = await this.arl.connect(this.unauthorized);

    // Crete ANOTHER_TOKEN-WAVAX pair
    await this.factory.createPair(this.wavax.address, this.another_token.address);
    let another_arl_address = await this.factory.getPair(this.wavax.address, this.another_token.address);
    this.another_arl = await this.Pair.attach(another_arl_address);
    this.alt_another_arl = await this.another_arl.connect(this.unauthorized);

    // Deploy ArcanaChef.
    this.chef = await this.Chef.deploy(this.arc.address, this.admin.address, this.factory.address, this.wavax.address);
    ...
    await this.arc.approve(this.router.address, ethers.constants.MaxUint256);
    ...
    await this.arc.approve(this.chef.address, ethers.constants.MaxUint256);
    await this.arl.approve(this.chef.address, ethers.constants.MaxUint256);
    await this.another_arl.approve(this.chef.address, ethers.constants.MaxUint256);
    ...
    await this.alt_arc.approve(this.router.address, ethers.constants.MaxUint256);
    ...
    await this.alt_arc.approve(this.chef.address, ethers.constants.MaxUint256);
    await this.alt_arl.approve(this.chef.address, ethers.constants.MaxUint256);
    await this.alt_another_arl.approve(this.chef.address, ethers.constants.MaxUint256);
    ...
    await this.arc.transfer(this.arl.address, ethers.utils.parseEther("1000000"));
    await this.wavax.transfer(this.arl.address, ethers.utils.parseEther("50000"));
    await this.arl.mint(this.admin.address);
    await this.another_token.transfer(this.another_arl.address, ethers.utils.parseEther("1000000"));
    await this.wavax.transfer(this.another_arl.address, ethers.utils.parseEther("50000"));
    await this.another_arl.mint(this.admin.address);

    // Get token amounts per address.
    this.arl_amount = (await this.arl.balanceOf(this.admin.address)).div("2");
    this.arc_amount = ARC_SUPPLY.div("2");

    // Transfer tokens to the second user.
    this.arl.transfer(this.unauthorized.address, this.arl_amount);
    this.arc.transfer(this.unauthorized.address, this.arc_amount);
    this.another_token.transfer(this.unauthorized.address, this.arc_amount);
  });
  ...
  describe("Constructor", function () {
    it("arg 1: rewardsToken", async function () {
      expect(await this.chef.rewardsToken()).to.equal(this.arc.address);
    });
    ...
    it("construction: no null inputs", async function () {
      ...
      await expect(this.Chef.deploy(this.arc.address, this.admin.address, this.factory.address, ethers.constants.AddressZero)).to.be.revertedWith("NullInput");
      await expect(this.Chef.deploy(this.arc.address, this.admin.address, ethers.constants.AddressZero, this.wavax.address)).to.be.reverted;
      await expect(this.Chef.deploy(this.arc.address, ethers.constants.AddressZero, this.factory.address, this.wavax.address)).to.be.revertedWith("NullInput");
    });
  ...
  describe("stake", function () {
    it("stakes to pool zero: reward added after staking", async function () {
      // We will stake four times, so divide the arl amount in wallet by 4.
      const amount = this.arl_amount.div("4");
    ...
    it("stakes to pool zero: reward added before staking", async function () {
      const amount = this.arl_amount.div("2");
      expect(await this.chef.addReward(ethers.utils.parseEther("1000000"))).to.emit(this.chef, "RewardAdded")
      expect(await this.chef.stake("0", amount)).to.emit(this.chef, "Staked");
      expect(await this.chef.stake("0", amount)).to.emit(this.chef, "Staked");
    });
  ...
  describe("harvest", function () {
    ...
    it("harvests from pool zero", async function () {
      ...
      await network.provider.send("evm_increaseTime", [PERIOD_DURATION]);
      expect(await this.chef.harvest("0")).to.emit(this.chef, "Withdrawn");
      expect(await this.chef.withdraw("0", this.arl_amount)).to.emit(this.chef, "Withdrawn");
    });
  });
  ...
  describe("compound", function () {
    it("compounds pool zero using AVAX", async function () {
      ...
      // Stake.
      expect(await this.chef.stake("0", this.arl_amount)).to.emit(this.chef, "Staked");
    ...
    it("compounds another pool using WAVAX", async function () {
      ...
      // Initialize the second pool. And add weight to the second pool.
      expect(await this.chef.initializePool(this.arl.address, "1")).to.emit(this.chef, "PoolInitialized");
      expect(await this.chef.setWeights(["1"], ["500"])).to.emit(this.chef, "WeightSet");

      // Stake.
      expect(await this.chef.stake("1", this.arl_amount)).to.emit(this.chef, "Staked");
  ...
  describe("compoundToPoolZero", function () {
    it("compounds to pool zero using AVAX", async function () {
      ...
      // Initialize the second pool. And add weight to the second pool.
      expect(await this.chef.initializePool(this.another_arl.address, "1")).to.emit(this.chef, "PoolInitialized");
      expect(await this.chef.setWeights(["1"], ["500"])).to.emit(this.chef, "WeightSet");
