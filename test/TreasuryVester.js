...
describe("TreasuryVester.sol", function () {
  ...
  beforeEach(async function () {
    ...
    await this.arc.setMinter(this.vester.address);
  ...
  describe("Constructor", function () {
    it("arg 1: vesting token", async function () {
      expect(await this.vester.vestedToken()).to.equal(this.arc.address);
    });
  ...
  describe("distribute", function () {
    it("cannot distribute when vesting disabled", async function () {
      ...
      var oldSupply = await this.arc.totalSupply();
      ...
      var newSupply = await this.arc.totalSupply();
    ...
    it("cannot distribute before vesting cliff is over", async function () {
      ...
      var newerSupply = await this.arc.totalSupply();
    ...
    it("distributes to MiniChef", async function () {
      ...
      chef = await this.Chef.deploy(this.arc.address, this.admin.address);
      await chef.deployed();
      ...
      var chefBalance = await this.arc.balanceOf(chef.address);
  ...
  describe("Semi-Random Vesting Simulation", function () {
    it("sets up contract for vesting", async function () {
      ...
      this.chef = await this.Chef.deploy(this.arc.address, this.admin.address);
    ...
    for (let month = 0; month < 30; month++) {
      it("vests month " + (month + 1), async function () {
        ...
        let balance = await this.arc.totalSupply();
    ...
    it("fails vesting after 30th month", async function () {
      ...
      let balance = await this.arc.balanceOf(this.vester.address);
    ...
    it("recipients got correct allocation", async function () {
      for (let i = 0; i < this.recipients.length; i++) {
        let balance = await this.arc.balanceOf(this.recipients[i].account);
