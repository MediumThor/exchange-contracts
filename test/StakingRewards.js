...
describe('StakingRewards', function () {
    before(async function () {
        ...
        this.LpToken = await ethers.getContractFactory("Arc");
    ...
    beforeEach(async function () {
        ...
        this.lpManager = await this.LpManager.deploy(this.mockWavax.address, this.arc.address,
        ...
        this.lpManagerTreasury = await this.LpManager2.deploy(this.mockWavax.address, this.arc.address,
            this.treasury.address);
        await this.lpManagerTreasury.deployed()

        this.stakingRewards = await this.StakingRewards.deploy(this.arc.address, this.lpToken.address);
    ...
    describe("startVesting", function () {
        it('Start vesting successfully', async function () {
            ...
            await this.arc.transfer(this.stakingRewards.address, vestAmount);
            ...
            expect(await this.arc.balanceOf(this.addr2.address)).to.equal(actualAmount);
        ...
        it('Two stakers, same time', async function () {
            ...
            expect(await this.arc.balanceOf(this.addr2.address)).to.equal(actualAmount);
            expect(await this.arc.balanceOf(this.addr3.address)).to.equal(actualAmount);
        ...
        it('Two stakers, different times', async function () {
            ...
            await this.stake2.getReward();
            await this.stake3.getReward();
            expect(await this.arc.balanceOf(this.addr2.address)).to.equal(largerAmount);
            expect(await this.arc.balanceOf(this.addr3.address)).to.equal(smallerAmount);
        });
        ...
        it('Early withdraw, the whole second period', async function () {
            ...
            expect(await this.arc.balanceOf(this.addr2.address)).to.equal(smallerAmount);
