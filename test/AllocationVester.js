...
describe("AllocationVester.sol", function () {
  ...
  beforeEach(async function () {
    // deploy ARC and send TOTAL_SUPPLY to admin
    this.arc = await this.Arc.deploy(
      TOTAL_SUPPLY,
      TOTAL_SUPPLY,
      "ARC",
      "Arcanum"
    ...
    this.vester = await this.Vester.deploy(this.arc.address);
  ...
  describe("Constructor", function () {
    it("arg 1: distributionToken", async function () {
      expect(await this.vester.token()).to.equal(this.arc.address);
    });
  });
  ...
  describe("setAllocations", function () {
    it("revert: insufficient arc balance for allocation", async function () {
      var [members, allocations, durations] = generateRecipients(1);
      var totalAlloc = arraySum(allocations);

      expect(
        await this.arc.transfer(this.vester.address, totalAlloc.sub("1"))
    ...
    it("revert: recipient is zero address", async function () {
      ...
      expect(await this.arc.transfer(this.vester.address, TOTAL_SUPPLY)).to.emit(
        this.arc,
        "Transfer"
      );
      await expect(
    ...
    it("revert: rate is zero", async function () {
      expect(
        await this.arc.transfer(this.vester.address, TOTAL_SUPPLY)
      ).to.emit(this.arc, "Transfer");
    ...
    it("revert: low balance when adding new members", async function () {
      ...
      expect(await this.arc.transfer(this.vester.address, totalAlloc)).to.emit(
        this.arc,
        "Transfer"
      );
      expect(
  ...
  describe("withdraw", function () {
    it("revert: insufficient arc balance", async function () {
      var [members, allocations, durations] = generateRecipients(1);
      var totalAlloc = arraySum(allocations);
    ...
    it("success: sufficient balance", async function () {
      ...
      expect(
        await this.arc.transfer(this.vester.address, totalAlloc.add("1"))
      ).to.emit(this.arc, "Transfer");
      expect(
        await this.vester.setAllocations(members, allocations, durations)
      ).to.emit(this.vester, "AllocationSet");
      expect(await this.vester.withdraw("1")).to.emit(this.arc, "Transfer");
    });
  });
  ...
  describe("members", function () {
    it("expect: updates on first allocation", async function () {
      ...
      expect(await this.arc.transfer(this.vester.address, allocations[0])).to.emit(
        this.arc,
        "Transfer"
      );
      expect(
