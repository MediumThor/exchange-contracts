...
describe("BridgeMigrationRouter", async function () {
    ...
    type ARCPairsTokenSymbol = keyof typeof fixture.Pairs.ARC
    type ARCMigratedPairsTokenSymbol = keyof typeof fixture.Pairs.Migrated.ARC
    ...
    before(async () => {
        ...
        const factory = await ethers.getContractAt("ArcanumFactory", fixture.Factory)
        const router = await ethers.getContractAt("ArcanumRouter", fixture.Router)

        await fundWAVAX(owner, BigNumber.from(10).pow(28))
        await fundToken(owner, fixture.Tokens.ARC, BigNumber.from(10).pow(25))
        await (await getTokenContract(fixture.Tokens.WAVAX)).approve(router.address, ethers.constants.MaxUint256)
        await (await getTokenContract(fixture.Tokens.ARC)).approve(router.address, ethers.constants.MaxUint256)
        ...
        // if there's no pairs in the migrators, create the pairs and add liquidity for ARC pairs
        for (let tokenSymbol of Object.keys(fixture.Pairs.Migrated.ARC)) {
            let tokenAddress = fixture.Migrators[tokenSymbol as MigratorTokenSymbol]
            if (!tokenAddress) continue
            if (fixture.Pairs.Migrated.ARC[tokenSymbol as ARCMigratedPairsTokenSymbol] !== "") {
                continue
            }
            const price = BigNumber.from("1000000000000000000")
            await router.connect(owner).addLiquidity(
                fixture.Tokens.ARC, tokenAddress,
                price, price.mul(2), price, price.mul(2),
                owner.address, getDeadline()
            )

            let bn0 = BigNumber.from(fixture.Tokens.ARC)
            ...
            fixture.Pairs.Migrated.ARC[tokenSymbol as ARCMigratedPairsTokenSymbol] = await factory.getPair(token0, token1)
        }
    })
    ...
    beforeEach(async () => {
        ...
        factory = await ethers.getContractFactory("ArcanumBridgeMigrationRouter")
    ...
    describe("Liquidity Migration", async function () {
        ...
        describe("ARC", async function () {
            for (let tokenSymbol of Object.keys(fixture.Migrators)) {
                let tokenAddress = fixture.Tokens[tokenSymbol as TokenSymbol]
                if (!((tokenSymbol as AVAXPairsTokenSymbol) in fixture.Pairs.ARC)) continue
                if (fixture.TokensWithoutFund.includes(tokenSymbol)) continue
                it(`Can migrate liquidity from ARC-${tokenSymbol}`, async function () {
                    let pairAddress = fixture.Pairs.ARC[tokenSymbol as ARCPairsTokenSymbol]
                    let toPairAddress = fixture.Pairs.Migrated.ARC[tokenSymbol as ARCPairsTokenSymbol]
                    let migrator = fixture.Migrators[tokenSymbol as MigratorTokenSymbol]
                    await migrationRouter.connect(owner).addMigrator(tokenAddress, migrator)
                    let liquidityAmount = await fundLiquidityToken(account, pairAddress, BigNumber.from(10).pow(20))
                    let fromPairContract = await getTokenContract(pairAddress)
                ...
                it(`Can compute accurately chargeback from ARC-${tokenSymbol}`, async function () {
                    let pairAddress = fixture.Pairs.AVAX[tokenSymbol as AVAXPairsTokenSymbol]
                    let toPairAddress = fixture.Pairs.Migrated.AVAX[tokenSymbol as AVAXMigratedPairsTokenSymbol]
                    let migrator = fixture.Migrators[tokenSymbol as MigratorTokenSymbol]
                    await migrationRouter.connect(owner).addMigrator(tokenAddress, migrator)
