...
contract RewarderViaMultiplierForArcanaChef is IRewarder {
    ...
    function onReward(
        ...
        for (uint256 i; i < rewardTokens.length; ++i) {
            uint256 pendingReward = rewardDebts[user][i].add(
                rewardAmount.mul(rewardMultipliers[i]) /
    ...
    function pendingTokens(
        ...
        address user,
        uint256 rewardAmount
    ) external view returns (IERC20[] memory tokens, uint256[] memory amounts) {
