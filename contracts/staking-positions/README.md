# Arcanum Staking Positions and Arcana Chef

Arcanum Staking Positions and Arcana Chef are unique staking and farming solutions. They utilize
...
* [`ArcanumStakingPositions.sol`](./ArcanumStakingPositions.sol)
* [`ArcanumStakingPositionsFunding.sol`](./ArcanumStakingPositionsFunding.sol)
* [`ArcanaChef.sol`](./ArcanaChef.sol)
* [`ArcanaChefFunding.sol`](./ArcanaChefFunding.sol)
...
### `ArcanumStakingPositions`

[`ArcanumStakingPositions`](./ArcanumStakingPositions.sol) is a single-sided staking solution in
which both the staking and reward tokens are the same token (i.e.: ARC). The rewards come from AMM
revenue. The revenue tokens get converted to ARC through `FeeCollector` (`SushiMaker` equivalent),
and then ARC is added to `ArcanumStakingPositions` as reward. In this implementation of SAR, we also
...
`ArcanumStakingPositions` is written specifically for ARC, which has a total supply of
`230_000_000e18`, and reverts on failed transactions.

### `ArcanaChef`

[`ArcanaChef`](./ArcanaChef.sol) is a MiniChef analogue that uses SAR algorithm.
...
ArcanaChef distributes global rewards to pools based on Synthetix’s staking algorithm, such that
...
ArcanaChef requires a Uniswap V2 factory and a wrapped native token address to be defined in
the constructor. Although it accepts any ERC20 token to be staked, it is mainly intended for
Arcanum liquidity pool tokens.

ArcanumStakingPosition had a simple compounding mechanism. In ArcanaChef, compounding requires
...
ArcanaChef, pool zero (`poolId == 0`) is reserved for `WRAPPED_NATIVE_TOKEN-REWARDS_TOKEN` liquidity
...
Another feature of ArcanaChef is rewarder. Rewarder is an external contract that can be defined
...
Yet another feature of ArcanaChef is relayer pools. That is an alternative type of pool to ERC20 pools,
...
`ArcanaChef` is mainly written for ArcanumPair tokens. The total staked amount limit of
...
though ARC supply fits 96 bits, the limit can theoretically be reached by rewards getting cycled
