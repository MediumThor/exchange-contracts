...
interface IArcanumStakingPositions {
...
contract TokenMetadata is AccessControlEnumerable {
    ...
    string public description =
        "This NFT perpetually receives share of the revenue generated by Arcanum. The share of the position is positively correlated to its staked balance and staking duration.";
    string public externalUri = "https://app.arcanum.exchange/#/stakev2";
    string public regularFontUri =
        "https://fonts.gstatic.com/s/poppins/v20/pxiEyp8kv8JHgFVrJJfecg.woff2";
    string public boldFontUri =
        "https://fonts.gstatic.com/s/poppins/v20/pxiByp8kv8JHgFVrLCz7Z1xlFQ.woff2";
    string public imageBaseUri =
        "https://static.pangolin.exchange/panguardian/";
    ...
    function setThousandSeparator(bytes1 newThousandSeparator)
        external
    ...
    function setExternalUri(string memory newExternalUri)
        external
        onlyRole(METADATA_ROLE)
    {
    ...
    function setFont(
        string memory newRegularFontUri,
        string memory newBoldFontUri
    ...
    function setImage(
        ...
        string memory newImageExtension
    ) external onlyRole(METADATA_ROLE) {
    ...
    function tokenURI(
        IArcanumStakingPositions arcanumStakingPositions,
        uint256 positionId
    ) external view returns (string memory) {
        IArcanumStakingPositions.ValueVariables
            memory positionValueVariables = arcanumStakingPositions
                .positions(positionId)
                .valueVariables;

        uint256 balance = positionValueVariables.balance;
        uint256 entryTime = balance == 0
            ? block.timestamp
            : positionValueVariables.sumOfEntryTimes / balance;
        uint256 apr = balance == 0
            ? 0
            : (arcanumStakingPositions.positionRewardRate(positionId) *
                365 days *
                100) / balance;
        uint256 earned = parseEther(
            arcanumStakingPositions.positionPendingRewards(positionId)
        );
        balance = parseEther(balance);

        uint256 balanceLevel = getExponent(balance, 404); // level 6 is reached at ~60k ARC.
        uint256 durationLevel = getExponent(
            (block.timestamp - entryTime),
            2 hours + 40 minutes
        ); // level 10 is reached in ~900 days.
    ...
    function addThousandSeparator(uint256 value)
        private
        view
        returns (string memory)
    {
        ...
        uint256 separatorsLength = digitsLength <= 3
            ? 0
            : (digitsLength - 1) / 3;
    ...
    function getExponent(uint256 value, uint256 multiplier)
        private
        view
        returns (uint256)
    {
