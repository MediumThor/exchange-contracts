...
import "./ArcanaChef.sol";

contract RewarderViaMultiplierForArcanaChefOnSongbird is IRewarder {
    ...
    mapping(uint256 => mapping(address => ArcanaChef.ValueVariables))
        private usersValueVariables;

    /// @param _rewardTokens The address of each additional reward token
    /// @param _rewardMultipliers The amount of each additional reward token to be claimable for every 1 base reward (ARC) being claimed
    /// @param _baseRewardTokenDecimals The decimal precision of the base reward (ARC) being emitted
    /// @param _chefV2 The address of the chef contract where the base reward (ARC) is being emitted
    ...
    constructor(
        ...
        for (uint256 i; i < _rewardTokens.length; ++i) {
            require(
                address(_rewardTokens[i]).code.length != 0,
                "RewarderViaMultiplier::Cannot reward zero address"
            );
            require(
                _rewardMultipliers[i] > 0,
                "RewarderViaMultiplier::Invalid multiplier"
            );
        }
    ...
    function onReward(
        ...
        // In Songbird ArcanaChef, `compoundToPoolZero` calls `onReward` with `destructiveAction == true`. This is invalid.
        // The following block detects the improper `destructiveAction == true`, and overrides it with `false`.
        ArcanaChef.ValueVariables memory newUserValueVariables = ArcanaChef(
            CHEF_V2
        ).getUser(pid, user).valueVariables;
        ArcanaChef.ValueVariables
            memory previousUserValueVariables = usersValueVariables[pid][user];
        usersValueVariables[pid][user] = newUserValueVariables;
        if (destructiveAction && newBalance != 0) {
            bool equalBalance = newUserValueVariables.balance ==
                previousUserValueVariables.balance;
            bool equalSumOfEntryTimes = newUserValueVariables.sumOfEntryTimes ==
                previousUserValueVariables.sumOfEntryTimes;
            if (
                previousUserValueVariables.balance == 0 ||
                (equalBalance && equalSumOfEntryTimes)
            ) destructiveAction = false;
        }

        for (uint256 i; i < rewardTokens.length; ++i) {
            uint256 pendingReward = rewardDebts[user][i] +
                ((rewardAmount * rewardMultipliers[i]) /
                    BASE_REWARD_TOKEN_DIVISOR);
    ...
    function pendingTokens(
        ...
        address user,
        uint256 rewardAmount
    ) external view returns (ERC20[] memory tokens, uint256[] memory amounts) {
    ...
    modifier onlyMCV2() {
        require(msg.sender == CHEF_V2, "Only MCV2 can call this function.");
        _;
    }
}
