...
contract MiniChefV2 is Ownable {
    ...
    /// @notice Address of reward (ARC) contract.
    ...
    constructor(address _rewardToken, address _firstOwner) public {
        require(
            _rewardToken != address(0) && _firstOwner != address(0),
    ...
    function setPools(
        ...
        IRewarder[] calldata rewarders,
        bool[] calldata overwrites
    ) external onlyOwner {
        require(
    ...
    function migrate(uint256 _pid) public onlyOwner {
        ...
        require(
            ...
            "MiniChefV2: migrated balance must match"
        );
        ...
        returns (uint256 pending)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 reward = time.mul(rewardPerSecond).mul(pool.allocPoint) /
                totalAllocPoint;
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp <= rewardsExpiration
                    ...
                    ? rewardsExpiration.sub(pool.lastRewardTime) // Accrue rewards until expiration
                    : 0; // No rewards to accrue
                uint256 reward = time.mul(rewardPerSecond).mul(
                    pool.allocPoint
                ) / totalAllocPoint;
                pool.accRewardPerShare = pool.accRewardPerShare.add(
                    (reward.mul(ACC_REWARD_PRECISION) / lpSupply).to128()
                );
            }
    ...
    function deposit(
        ...
        user.rewardDebt = user.rewardDebt.add(
            int256(amount.mul(pool.accRewardPerShare) / ACC_REWARD_PRECISION)
        );
    ...
    modifier onlyFunder() {
        require(
            msg.sender == owner() || funder[msg.sender] == true,
            "MiniChefV2: caller is not a funder"
        );
        _;
    }
    ...
    function fundRewards(uint256 funding, uint256 duration)
        external
        onlyFunder
    {
        ...
        if (block.timestamp >= rewardsExpiration) {
            ...
            uint256 newRewardPerSecond = remainingRewards.add(funding) /
                (newRewardsExpiration.sub(block.timestamp));
    ...
    function extendRewardsViaFunding(uint256 funding, uint256 minExtension)
        external
    {
        ...
        require(
            extensionDuration >= minExtension,
            "MiniChefV2: insufficient extension limit"
        );
    ...
    function extendRewardsViaDuration(uint256 extension, uint256 maxFunding)
        external
    {
        ...
        require(
            fundingRequired <= maxFunding,
            "MiniChefV2: insufficient funding limit"
        );
