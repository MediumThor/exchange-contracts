...
import "../arcanum-core/interfaces/IArcanumERC20.sol";
import "../arcanum-lib/libraries/TransferHelper.sol";
import "./interfaces/IBridgeToken.sol";
import "./libraries/Roles.sol";
import "./libraries/ArcanumLibrary.sol";

contract ArcanumBridgeMigrationRouter {
    ...
    modifier ensure(uint256 deadline) {
        require(
            ...
            "ArcanumBridgeMigrationRouter: EXPIRED"
        );
        _;
    }
    ...
    modifier onlyAdmin() {
        require(
            ...
            "ArcanumBridgeMigrationRouter: Unauthorized"
        );
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(
            account != address(0),
            "ArcanumBridgeMigrationRouter: Address 0 not allowed"
        );
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(
            msg.sender != account,
            "ArcanumBridgeMigrationRouter: You can't demote yourself"
        );
        ...
        onlyAdmin
    {
        require(
            ...
            "ArcanumBridgeMigrationRouter: tokenAddress 0 not supported"
        );
        require(
            migratorAddress != address(0),
            "ArcanumBridgeMigrationRouter: migratorAddress 0 not supported"
        );
    ...
    function _allowToken(address tokenAddress, address spenderAddress)
        internal
    {
        IArcanumERC20(tokenAddress).approve(spenderAddress, type(uint256).max);
    }
    ...
    {
        (uint112 reserve0, uint112 reserve1, ) = IArcanumPair(pairToken)
            .getReserves();
        uint256 quote0 = amountIn0;
        uint256 quote1 = ArcanumLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = ArcanumLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IArcanumPair(pairToken).mint(to);
    }
        ...
        returns (uint256 amountTokenA, uint256 amountTokenB)
    {
        ...
        (amountTokenA, amountTokenB) = IArcanumPair(liquidityPair).burn(
            address(this)
        );
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(
            pairA != address(0),
            "ArcanumBridgeMigrationRouter: liquidityPairFrom address 0"
        );
        require(
            pairA != address(0),
            "ArcanumBridgeMigrationRouter: liquidityPairTo address 0"
        );
        require(
            pairA != pairB,
            "ArcanumBridgeMigrationRouter: Cant convert to the same liquidity pairs"
        );
        require(
            IArcanumPair(pairA).token0() == IArcanumPair(pairB).token0() ||
                IArcanumPair(pairA).token0() ==
                IArcanumPair(pairB).token1() ||
                IArcanumPair(pairA).token1() ==
                IArcanumPair(pairB).token0() ||
                IArcanumPair(pairA).token1() == IArcanumPair(pairB).token1(),
            "ArcanumBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(address tokenAddress, uint256 amount) internal {
        ...
        require(
            ...
            "ArcanumBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "ArcanumBridgeMigrationRouter: migrator not registered"
        );
        TransferHelper.safeTransferFrom(
            token,
            msg.sender,
    ...
    function migrateLiquidityWithPermit(
        ...
        IArcanumPair(liquidityPairFrom).permit(
            msg.sender,
            address(this),
            amount,
            deadline,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IArcanumPair(liquidityPairFrom).token0();
        if (
            IArcanumPair(liquidityPairFrom).token0() ==
            IArcanumPair(liquidityPairTo).token0() ||
            IArcanumPair(liquidityPairFrom).token0() ==
            IArcanumPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IArcanumPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "ArcanumBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IArcanumPair(liquidityPairTo).token0() ||
                newTokenAddress == IArcanumPair(liquidityPairTo).token1(),
            "ArcanumBridgeMigrationRouter: Pair doesn't match the migration token"
        );
        ...
        );
        {
            ...
            if (tokenToMigrate != IArcanumPair(liquidityPairFrom).token0()) {
        ...
        if (
            IArcanumPair(liquidityPairFrom).token0() !=
            IArcanumPair(liquidityPairTo).token0() &&
            IArcanumPair(liquidityPairFrom).token1() !=
            IArcanumPair(liquidityPairTo).token1()
        ) {
    ...
    {
        (uint112 reserves0, uint112 reserves1, ) = IArcanumPair(pairAddress)
            .getReserves();
        uint256 totalSupply = IArcanumPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(
            ...
            "ArcanumBridgeMigrationRouter: liquidityPairFrom address 0 not supported"
        );
        require(
            liquidityPairTo != address(0),
            "ArcanumBridgeMigrationRouter: liquidityPairTo address 0 not supported"
        );
        ...
        (uint112 reserve0, uint112 reserve1, ) = IArcanumPair(liquidityPairTo)
            .getReserves();
        uint256 quote0 = amountIn0;
        uint256 quote1 = ArcanumLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
